...
//	Category section
  ...
  public String saveProduct(@ModelAttribute("product") productDto productDTO,
	  ...
	  products.setPrice(productDTO.getPrice());
		...
		/*
		 * String fileName; if(!file.isEmpty()) {
		 * 
		 * 
		 * 
		 * StringBuilder filename=new StringBuilder();
		 * fileName=productDTO.getProduct_id()+file.getOriginalFilename().substring(file
		 * .getOriginalFilename().length()-4); java.nio.file.Path
		 * fileNameAndPath=Paths.get(uploadDir, fileName); Files.write(fileNameAndPath,
		 * file.getBytes());
		 * 
